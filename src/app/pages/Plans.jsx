...
function Plans() {
  ...
  return <div className="py-10 px-4 sm:px-6 lg:px-8">
  ...
  >
        ...
        <div className="space-y-10">
          <section>
            ...
            {!loading && !error && upcomingEvents.length === 0 && <p className={`mt-4 text-sm ${isDarkTheme ? "text-slate-300" : "text-slate-500"}`}>
    ...
    const cfg = eventTypeConfig[event.type] ?? {
      badge: isDarkTheme ? "bg-white/5 text-slate-200 border border-white/20" : "bg-slate-100 text-slate-700 border border-slate-200",
      border: isDarkTheme ? "border-slate-400/60" : "border-slate-300/80"
    };
    return <ScrollReveal key={event._id || `${event.title}-${index}`} delay={index * 60}>
                    <article
      className={`group rounded-2xl border p-5 h-full transition-all duration-300 ${isDarkTheme ? "border-white/10 bg-white/[0.03] hover:-translate-y-0.5 hover:border-white/20 hover:bg-white/[0.05]" : "border-slate-200/90 bg-[linear-gradient(165deg,rgba(255,255,255,0.94),rgba(241,245,249,0.96))] hover:-translate-y-0.5 hover:shadow-[0_24px_62px_-42px_rgba(14,165,233,0.35)]"}`}
    >
                      ...
                      <Link
      to="/contact"
      className={`mt-4 inline-flex items-center gap-1.5 rounded-full px-4 py-2 text-sm font-medium transition-all active:scale-95 ${isDarkTheme ? `border-l-2 ${cfg.border} border border-white/15 bg-white/[0.04] text-white hover:bg-white/[0.1]` : "border border-sky-300/70 bg-sky-100 text-sky-700 hover:bg-sky-200"}`}
    >
                        Join Plan
                        <ArrowRight size={14} />
                      </Link>
          ...
          <section>
            ...
            {!loading && !error && recurringSchedule.length === 0 && <p className={`mt-4 text-sm ${isDarkTheme ? "text-slate-300" : "text-slate-500"}`}>
                Recurring tracks will appear here once added from admin.
              </p>}

            <div className="mt-4 grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
              {recurringSchedule.map((item, index) => <ScrollReveal key={item._id || `${item.title}-${index}`} delay={index * 70}>
                  <article
    className={`group rounded-2xl border p-5 h-full transition-all duration-300 ${isDarkTheme ? "border-white/10 bg-white/[0.03] hover:-translate-y-0.5 hover:border-sky-300/40 hover:bg-white/[0.05]" : "border-slate-200/90 bg-[linear-gradient(165deg,rgba(255,255,255,0.94),rgba(241,245,249,0.96))] hover:-translate-y-0.5 hover:shadow-[0_24px_62px_-42px_rgba(14,165,233,0.35)]"}`}
  >
                    ...
                    <button
    className={`mt-4 text-sm font-medium inline-flex items-center gap-1.5 transition-colors ${isDarkTheme ? "text-sky-200 hover:text-white" : "text-sky-700 hover:text-sky-800"}`}
  >
                      Join Track
                      <ArrowRight size={14} />
                    </button>

...
const lightStatusConfig: Record<string, { badge: string; dot: string }> = {
...
const darkStatusConfig: Record<string, { badge: string; dot: string }> = {
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {!loading && !error && filtered.map((project, index) => {
            const s = statusConfig[project.status] ?? {
              badge: isDarkTheme
                ? "bg-slate-500/15 text-slate-100 border border-slate-300/25"
                : "bg-gray-50 text-gray-700 border border-gray-200",
              dot: isDarkTheme ? "bg-slate-300" : "bg-gray-400",
            };
            return (
              <ScrollReveal key={project._id} delay={(index % 3) * 80}>
                <div className={`group rounded-2xl overflow-hidden transition-all duration-300 flex flex-col h-full hover:-translate-y-1 ${isDarkTheme
                  ? "bg-[linear-gradient(170deg,rgba(2,6,23,0.95),rgba(15,23,42,0.93))] border border-white/10 hover:shadow-[0_26px_68px_-44px_rgba(14,165,233,0.62)]"

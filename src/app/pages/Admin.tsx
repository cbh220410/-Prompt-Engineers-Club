...
import { api, HomeContent, HomeFeature, Plan, Project, TeamMember } from "../lib/api";
...
const emptyHomeFeatureDraft: HomeFeature = {
  icon: "Lightbulb",
  title: "",
  desc: "",
  link: "",
...
export function Admin() {
  ...
  const [home, setHome] = useState<HomeContent>(emptyHomeDraft);
  ...
  const [homeFeatureDraft, setHomeFeatureDraft] = useState<HomeFeature>(emptyHomeFeatureDraft);
  const [homeFeatureEditIndex, setHomeFeatureEditIndex] = useState<number | null>(null);
    ...
    return (
      <div className="max-w-md mx-auto px-4 py-24">
        <div className="rounded-2xl border border-white/10 bg-slate-950/80 p-6 space-y-4">
          ...
          {passwordError && (
            <p className="text-sm text-red-300 bg-red-950/30 border border-red-500/40 rounded-lg px-3 py-2">
              {passwordError}
            </p>
          )}
        </div>
  ...
  const saveHomeFeatureDraft = (e: FormEvent) => {
    e.preventDefault();
    resetMsg();

    const normalized: HomeFeature = {
  ...
  const startEditHomeFeature = (index: number) => {
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        <form onSubmit={saveTeam} className="rounded-2xl border border-white/10 bg-slate-900/70 p-4 space-y-3">
          ...
          <div className="flex gap-2">
            ...
            {teamEditId && <button type="button" onClick={() => { setTeamEditId(null); setTeamDraft(emptyTeamDraft); }} className="px-4 py-2 rounded-lg border border-white/20">Cancel</button>}
          </div>
          <div className="space-y-2 pt-2 border-t border-white/10">
            {team.map((member) => (
              <div key={member._id} className="rounded-lg border border-white/10 p-2 flex items-center justify-between gap-2">
                <div>
                  <p className="text-sm text-white font-medium">{member.name}</p>
                  <p className="text-xs text-slate-300">{member.role}</p>
                </div>
                <div className="flex gap-2">
                  <button type="button" onClick={() => { setTeamEditId(member._id); setTeamDraft({ ...member }); }} className="px-2 py-1 rounded border border-white/20 text-xs">Edit</button>
                  <button type="button" onClick={async () => { await api.deleteTeamMember(member._id); setTeam(await api.getTeam()); await refreshSeedCode(); }} className="px-2 py-1 rounded border border-red-400/40 text-xs text-red-300">Delete</button>
                </div>
              </div>
            ))}
          </div>
        ...
        <form onSubmit={savePlan} className="rounded-2xl border border-white/10 bg-slate-900/70 p-4 space-y-3">
          ...
          <div className="flex gap-2">
            ...
            {planEditId && <button type="button" onClick={() => { setPlanEditId(null); setPlanDraft(emptyPlanDraft); }} className="px-4 py-2 rounded-lg border border-white/20">Cancel</button>}
          </div>
          <div className="space-y-2 pt-2 border-t border-white/10 max-h-56 overflow-auto">
            {plans.map((plan) => (
              <div key={plan._id} className="rounded-lg border border-white/10 p-2 flex items-center justify-between gap-2">
                <div>
                  <p className="text-sm text-white font-medium">{plan.title}</p>
                  <p className="text-xs text-slate-300">{plan.type}</p>
                </div>
                <div className="flex gap-2">
                  <button type="button" onClick={() => { setPlanEditId(plan._id); setPlanDraft({ ...plan }); }} className="px-2 py-1 rounded border border-white/20 text-xs">Edit</button>
                  <button type="button" onClick={async () => { await api.deletePlan(plan._id); setPlans(await api.getPlans()); await refreshSeedCode(); }} className="px-2 py-1 rounded border border-red-400/40 text-xs text-red-300">Delete</button>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        <form onSubmit={saveProject} className="rounded-2xl border border-white/10 bg-slate-900/70 p-4 space-y-3">
          ...
          <div className="flex gap-2">
            ...
            {projectEditId && <button type="button" onClick={() => { setProjectEditId(null); setProjectDraft(emptyProjectDraft); }} className="px-4 py-2 rounded-lg border border-white/20">Cancel</button>}
          </div>
          <div className="space-y-2 pt-2 border-t border-white/10 max-h-56 overflow-auto">
            {projects.map((project) => (
              <div key={project._id} className="rounded-lg border border-white/10 p-2 flex items-center justify-between gap-2">
                <div>
                  <p className="text-sm text-white font-medium">{project.title}</p>
                  <p className="text-xs text-slate-300">{project.status}</p>
                </div>
                <div className="flex gap-2">
                  <button type="button" onClick={() => { setProjectEditId(project._id); setProjectDraft({ ...project, tags: project.tags || [] }); }} className="px-2 py-1 rounded border border-white/20 text-xs">Edit</button>
                  <button type="button" onClick={async () => { await api.deleteProject(project._id); setProjects(await api.getProjects()); await refreshSeedCode(); }} className="px-2 py-1 rounded border border-red-400/40 text-xs text-red-300">Delete</button>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="rounded-xl border border-white/10 bg-slate-950/60 p-3 space-y-3">
            ...
            <div className="space-y-2 border-t border-white/10 pt-2 max-h-56 overflow-auto">
              {homeFeatures.map((feature, index) => (
                <div key={`${feature.title}-${index}`} className="rounded-lg border border-white/10 p-2 flex items-start justify-between gap-2">
                  <div>
              ...
              {homeFeatures.length === 0 && (
                <p className="text-xs text-slate-400">No future plan cards added yet.</p>
              )}
            </div>
          </div>
